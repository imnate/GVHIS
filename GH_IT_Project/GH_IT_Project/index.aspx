...
<body>
                ...
                <div class="col-md-3">
                    <div class="navbar navbar-default">
                        ...
                        <p>
                            ...
                            <a href="https://care.vac.gov.tw/wevac/"><i class="fas fa-file-alt"></i>榮家安養養護管理資訊系統</a><br>
                            <a href="https://care.vac.gov.tw/wevac3/"><i class="fas fa-file-alt"></i>服務績效管理系統</a><br>
                            ...
                            switch (data[0].authority) {
                                case "s":
                                    ...
                                    $(management).append(
                                        ...
                                        '<li><a data-toggle="modal" data-target="#Secretary_management_Modal" id="Secretary_management" href="#" onclick = "FeedBackMessageNum()"><i class="fas fa-lock-open"></i>  秘書室管理</a></li>' +
    ...
    <script>    
        function FeedBackMessageNum() {
            Ss_Msg_Board.ajax.reload();
            //執行ajax進行抓取未回覆留言數量，並且刪除兩個月以前的留言
            $.ajax({
                url: "FeedBackMsgNum.asmx/FeedBackMsg",
                method: "post",
                dataType: "json",
                success: function (data) {
                    //假如data數字大於0就是有新留言未回覆 顯示comfire
                    //沒有就不顯示comfire
                    console.log("FeedBackMsgNum" + data);
                    if (data > 0) {
                        $.confirm({
                            icon: 'fas fa-comment',
                            title: '提醒',
                            content: '有 ' + data + ' 筆 留言未回覆，請立即前往並且回覆。',
                            type: 'orange',
                            typeAnimated: true,
                            buttons: {
                                correct: {
                                ...
                                check_Msg: {
                                    ...
                                    btnClass: 'btn-info',
                                    action: function () {
                                        $(Ss_MessageBoard_Modal).modal('show');
                                    }
                                },
                            }
                        });
            ...
            $.ajax({
                ...
                success: function (data) {
                    $.confirm({
                        ...
                        '<div class="form-group">' +
                        '<label>回覆:</label>' +
                        '<textarea rows="5" class="edit_MessageDetail_Feedback form-control" placeholder="例如:閱">' + data[0].feedback + '</textarea>' +
                        '</div>' +
                        '</form>',
                        type: 'orange',
                        //typeAnimated: true,
                        buttons: {
                            submit: {
                                ...
                                btnClass: 'btn-blue',
                                action: function () {
                                    if (this.$content.find('.edit_MessageDetail_Feedback').val().length > 0) {
                                        var ID = data[0].strId;
            ...
            <div class="modal-content">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3" style="text-align: left">
                        ...
                        <button type="button" id="Download_Year_Form" onclick="Generator_LeadersBigEvent_Form()" class="btn btn-danger">下載大事記</button>
                        <button type="button" class="btn btn-danger" onclick="DeleteOldData()"><i class="far fa-calendar-times"></i></button>
    ...
    <script>    
        function DeleteOldData() {
            $.confirm({
                ...
                '<div class="form-group">' +
                '<label>帳號:</label>' +
                '<input type="text" maxlength="15" class="edit_AuthAccount form-control">' +
                '</div>' +
                '<label>密碼:</label>' +
                '<input type="Password" maxlength="15" class="edit_AuthPassword form-control">' +
                '</div>' +
                '</form>',
                ...
                buttons: {
                    formSubmit: {
                        ...
                        btnClass: 'btn-green',
                        action: function () {
                            var Account = this.$content.find('.edit_AuthAccount').val();
                            var Password = this.$content.find('.edit_AuthPassword').val();
    ...
    <script>    
        function DeleteOlderBigEvent() {
            $.confirm({
                ...
                '<div class="form-group">' +
                '<label>刪除年份(西元):</label>' +
                '<input type="text" class="DeleteOldBigEventYear form-control" placeholder="例如:2018">' +
                '</div>' +
                '</form>',
                type: 'red',
                typeAnimated: true,
                buttons: {
                    correct: {
                        ...
                        action: function () {
                            //do ajax 找資料庫
                            var DeleteOldBigEventYear = this.$content.find('.DeleteOldBigEventYear').val();
                                $.ajax({
                                    url: "SearchNew_schedule.asmx/Schedule_searchByYear",
                                    method: "post",
                                    dataType: "json",
                                    data: {
                                        Year: DeleteOldBigEventYear
                                    },
                                    success: function (data) {
                                        console.log("DeleteOldBigEventYear :" + DeleteOldBigEventYear + "ToF :" + data[0].ToFYear + " listCount " + data[0].Count + " Output :" + data[0].Output);
                                        if (data[0].ToFYear == true && data[0].Count>0) {//DO DELETE
                                            $.confirm({
                                                icon: 'fas fa-exclamation-triangle',
                                                title: '確認刪除?',
                                                content: '刪除年份: ' + DeleteOldBigEventYear + '<br>' +
                                                '資料庫資料筆數: ' + data[0].Count + '筆',
                                                type: 'orange',
                                                typeAnimated: true,
                                                buttons: {
                                                    Delete: {
                                                        ...
                                                        action: function () {
                                                            $.ajax({
                                                                url: "SearchNew_schedule.asmx/DeletebyYear",
                                                                method: "post",
                                                                dataType: "json",
                                                                data: {
                                                                    Year: DeleteOldBigEventYear,
                                                                ...
                                                                success: function (data) {
                                                                    swal({
                                                                        title: "成功刪除!",
                                                                        text: '刪除了' + data + '筆' + DeleteOldBigEventYear+'資料',
                                                                        type: "success",
                        ...
                        },
                        {
                            ...
                            "render": function (data, type) {
                                if (type === 'display') {
                                    if (data.length > 13) {
                                        var result = data;
                                        var DataDivsion = (data.length + (data.length / 12) * 4) / 12;
                                        var c = 0;
                                        for (var i = 1; i <= DataDivsion; i++) {
                                            result = i == 1 ? result.substr(0, i * 11 + c * 1) + '<br>' + result.substr(i * 11 + c * 1, result.length) : result.substr(0, i * 11 + c * 4) + '<br>' + result.substr(i * 11 + c * 4, result.length);
                                            c++;
                                        }
                                        return '<div class="text-center" title="' + data + '">' + result + '</div></div>';
                        ...
                        },
                        {
                            ...
                            "render": function (data, type) {
                                if (type === 'display') {
                                    if (data.length > 10) {
                                        var result = data;
                                        var DataDivsion = (data.length + (data.length / 10) * 4) / 10;
                                        var c = 0;
                                        for (var i = 1; i <= DataDivsion; i++) {
                                            result = i == 1 ? result.substr(0, i * 10 + c * 1) + '<br>' + result.substr(i * 10 + c * 1, result.length) : result.substr(0, i * 10 + c * 4) + '<br>' + result.substr(i * 10 + c * 4, result.length);
                                            c++;
                                        }
                                        return '<div class="text-center" title="' + data + '">' + result + '</div></div>';
                                        //return '<div class="text-center" title="' + data + '">' + data.substr(0, 12) + '...</div>';
                ...
                success: function (data) {
                    ...
                    if (data[0].Time_TorF == false) {
                        $.confirm({
                            ...
                            + '<br><span style="font-weight:bold;color:green;">' + $(Sms_Modal_host).val() + ' 未排行程時間: (上班時間8:00-17:00)</span><br>' + data[0].Advice_Time,
                            type: 'red',
                            typeAnimated: true,
                            buttons: {
                                submit: {
    ...
    <script>       
        ...
        $(document).ready(function () {
            ...
            var TodaySystemFormat = Today.getFullYear() + "-" + MM + "-" + DD + " " + day_list[Today.getDay()];
            $('#date_picker_input').val(TodaySystemFormat);
        ...
        $(document).ready(function () {
            PRLD = $('#PR_Leaving_DataTable').DataTable({
                ...
                "oLanguage": {
                    "sProcessing": "處理中...",
                    "sLengthMenu": "顯示 _MENU_ 筆記錄",
                    "sZeroRecords": "無任何組室主管、堂隊長請假",
                    "sInfo": "出勤狀況筆數：_TOTAL_",
                    "sInfoEmpty": "無任何組室主管、堂隊長請假",
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="form-group">
                                <label for="message-text" class="form-control-label col-sm-2">故障說明:</label>
                                <div class="col-sm-12">
                        ...
                        "render": function (data, type) {
                            if (type === 'display') {
                                if (data.length > 10) {
                                    ...
                                    var c = 0;
                                    for (var i = 1; i <= DataDivsion; i++) {
                                        result = i == 1 ? result.substr(0, i * 9 + c * 1) + '<br>' + result.substr(i * 9 + c * 1, result.length) : result.substr(0, i * 9 + c * 4) + '<br>' + result.substr(i * 9 + c * 4, result.length);
                                        c++;
                                    }
                                    return '<div class="text-center" title="' + data + '">' + result + '</div></div>';
                                    //return '<div class="text-center" title="' + data + '">' + data.substr(0, 12) + '...</div>';
    ...
    <script>
                ...
                success: function (data) {
                    ...
                    if (data[0].Time_TorF == false) {
                        $.confirm({
                            ...
                            + '<br><span style="font-weight:bold;color:green;">' + $("#Host_select option:selected").text() + ' 未排行程時間: (上班時間8:00-17:00)</span><br>' + data[0].Advice_Time,
                            type: 'red',
                            typeAnimated: true,
                            buttons: {
                                tryAgain: {
        ...
        function insert_schedule() {//行程寫入AJAX
            console.log('行程時間: ' + $(new_Ss_dateAndtime_input).val());
            $.ajax({
                    ...
                    switch (data[0].outcase) {
                        case 1:
                            ...
                            $.confirm({
                                ...
                                '</div>' +
                                '<H3 style="text-align: center;"><span style="font-weight:bold;color:red;">警告</H3>' +
                                //'<div style="text-align: center; font-weight:bold;"><i class="fas fa-exclamation-circle">警告</i><br></div>' +
                                '<div style="text-align: center;">因距離新增的行程還剩下 ' + data[0].Days + '天 的時間<br></div>' +
                                '<div style="text-align: center;">為了避免行程沒有被審核，遵照以下:<br></div>' +
                                '<div style="text-align: center; font-weight:bold;">' +
                                '1.立即通知 <span style="text-align: center; text-decoration:underline;color:red;">秘書</span> 告知行程必須立即審核<br>' +
                                '2.或者在<span style="text-align: center; text-decoration:underline;color:red;">留言板</span>上告知行程必須立即審核<br></div>' +
                            ...
                            <div class="panel panel-primary">
                                ...
                                <div class="panel-body">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div><small class="form-text text-muted" style="font-weight: bold; color: red;">*請勿在此填寫故障說明</small></div>
                                        <div><small class="form-text text-muted" style="font-weight: bold; color: red;">*兩個以上報修地點請按上方橘色按鈕[新增項目]</small></div>
                                    </div>
                                    <div class="col-md-6">
                                        <label for="message-text" class="form-control-label">故障說明:</label>
    ...
    <script>
                    ...
                    "columns": [
                        ...
                        },
                        {
                            ...
                            "render": function (data, type) {
                                if (type === 'display') {
                                    if (data.length > 13) {
                                        return '<div class="text-center" title="' + data + '">' + data.substr(0, 4) + '...</div>';
                                    } else {
                                        return '<div class="text-center" title="' + data + '">' + data + '</div>';
                                    }
                                }
        ...
        function Generator_LeadersBigEvent_Form() {
            if ($('#SM_date_picker_input').val().length > 0) {
                console.log("搜尋年分:" + $('#SM_date_picker_input').val());
                $.confirm({
                    icon: 'fas fa-minus-circle',
                    title: '',
                    content: '<H3 style="text-align: center;"><span style="font-weight:bold;color:red;">注意</H3><br>' +
                    '<style="text-align: center;">一、請利用非上班(17:00過後)時間使用此功能<br><br>' +
                    '<style="text-align: center;">二、按下確定按鈕後不要再做任何操作，直到下載完Word檔案<br><br>',
                    //type: 'orange',
                    typeAnimated: true,
                    buttons: {
                        Correct: {
